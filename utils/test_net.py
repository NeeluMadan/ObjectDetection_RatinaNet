...
def test_model():
    ...
    for im in range(img_nums):
        ...
        # For visualization
        filename = os.path.join("visual_results",  img_list[im].strip() + '.jpg')
        ...
        if is_found :
            ...
            font = ImageFont.truetype(b'/usr/share/fonts/truetype/ancient-scripts/Symbola_hint.ttf', 20) 
 
            count = 0
            for box in boxes:
                draw.rectangle(list(box), outline='red', width=5)
                draw.text((box[0], box[1]-20),str(category[labels[count]]),font=font, fill=(255,0,0,255))
                count = count + 1
            img.save(filename)
